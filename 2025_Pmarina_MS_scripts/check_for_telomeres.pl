...
unless (-f $fasta){
	print "\nFasta file not found: $fasta \n";
	print "Please enter at least one FASTA file. Exiting...\n\n";
	exit;
}
...
my $out_check = $outdir.'/'.$prefix.'.telomeres_check.txt';
my $out_summary = $outdir.'/'.$prefix.'.telomeres_summary.txt';
my $out_kmers = $outdir.'/'.$prefix.'.telomeres_kmers.txt';
...
## GC percentage
my $gc_percentage = ($gc_count/$sum_len)*100;
...
foreach my $contig (sort (keys %telo_metrics)){
	...
	my $telosum = 0;
	my $partial_tel_left = 0;
	my $partial_tel_right = 0;
	...
	if ($telosum == 2){
		...
		if ($partial_tel_right == 0){
			$incomplete_end = 'right';
		}
		else {
			$complete_end = 'right';
		}

		print SUM "\t".'partial chromosome with 1 telomere end ('.$complete_end.'): '.$incomplete_end.' end is missing';
	}
	else {
