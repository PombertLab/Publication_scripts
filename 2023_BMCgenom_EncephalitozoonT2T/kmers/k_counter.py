...
-w (--window)		Window size to check for kmers [Default: 600]
-s (--slide)		Number of nucleotides to shift window over [Default: 1]
"""
...
GetOptions.add_argument('-w','--window',default=600)
GetOptions.add_argument('-s','--slide',default=1)
...
for orgs in in_files:
	...
	for line in IN:
		line = line.replace("\n","")
		if match("^\>(\w+)",line): 
			chromosome = match("^\>(\w+)",line).groups()[0]
