...
while(my $line = <GESAMT>){
	chomp($line);
	unless(($line =~ /^###/)||($line eq '')){
		my ($locus_tag,$predictor,$rcsb_code,$rcsb_chain,$qscore,$rmsd,$seqid,$nalign,undef,undef,$description) = split("\t",$line);
		$seqid = sprintf("%4.2f",$seqid*100);
		...
		if(($predictor eq 'ALPHAFOLD')&&($alphafold_counter < 6)){
			push(@{$alignment_scores{$locus}{$predictor}{"qscore"}},[$qscore,$description,$rcsb_code,$rcsb_chain,$rmsd,$nalign,$seqid]);
			$alphafold_counter++;
		}
...
print(("\tVoroCNN\tRCSB Code\tRCSB Chain\tQ-score\tRMSD\tAlign. Length\t% Id\tTMscore\tRMSD\tAlign. Length\t% Id\tGDT\tRMSD\tAlign. Length\tDescription"x2)."\n");
	...
	for (my $i = 0; $i < 6; $i++){
		...
		if($i == 0){
			...
			if($alignment_scores{$locus}{"ALPHAFOLD"}{"qscore"}){
				my ($qscore,$description,$rcsb_code,$rcsb_chain,$rmsd,$nalign,$seqid) = @{$alignment_scores{$locus}{"ALPHAFOLD"}{"qscore"}[0]};
				$line .= "\t".$rcsb_code."\t".$rcsb_chain."\t".$qscore."\t".$rmsd."\t".$nalign."\t".$seqid;
			...
			if($alignment_scores{$locus}{"RAPTORX"}{"qscore"}){
				my ($qscore,$description,$rcsb_code,$rcsb_chain,$rmsd,$nalign,$seqid) = @{$alignment_scores{$locus}{"RAPTORX"}{"qscore"}[0]};
				$line .= "\t".$rcsb_code."\t".$rcsb_chain."\t".$qscore."\t".$rmsd."\t".$nalign."\t".$seqid;
			...
			if($alignment_scores{$locus}{"ALPHAFOLD"}{"qscore"}[$i]){
				$line .= "\t"x4;
				my ($qscore,$description,$rcsb_code,$rcsb_chain,$rmsd,$nalign,$seqid) = @{$alignment_scores{$locus}{"ALPHAFOLD"}{"qscore"}[$i]};
				$line .= "\t".$rcsb_code."\t".$rcsb_chain."\t".$qscore."\t".$rmsd."\t".$nalign."\t".$seqid;
			...
			if($alignment_scores{$locus}{"RAPTORX"}{"qscore"}[$i]){
				$line .= "\t";
				my ($qscore,$description,$rcsb_code,$rcsb_chain,$rmsd,$nalign,$seqid) = @{$alignment_scores{$locus}{"RAPTORX"}{"qscore"}[$i]};
				$line .= "\t".$rcsb_code."\t".$rcsb_chain."\t".$qscore."\t".$rmsd."\t".$nalign."\t".$seqid;
