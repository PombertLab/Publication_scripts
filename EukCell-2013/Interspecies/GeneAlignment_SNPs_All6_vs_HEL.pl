...
my $usage = 'perl GeneAlignment_SNPs_All6_vs_HEL.pl list.txt';
	...
	open IN, "<$txt" or die "cannot open $txt";
		...
		while (my $line = <IN>) {
			...
			if ($line =~ /^(ECU\d+.*)/) { ## match the query to fit the desired gene names in the list provided
				...
				open INVAR, ">$ECU.hel.invar";
				open SNPs, ">$ECU.hel.snps";
				open GAPs, ">$ECU.hel.gaps";
				open MET, ">$ECU.hel.metrics";
				...
				for (0..($end-1)) {
					...
					if (($ECI[$_] eq '-') || ($ECII[$_] eq '-') || ($ECIII[$_] eq '-')|| ($HEL[$_] eq '-')) {
					print GAPs "$position\t$ECI[$_]\t$ECII[$_]\t$ECIII[$_]\t$HEL[$_]\n";
					$position++; 
					}
					elsif (($ECI[$_] eq $ECII[$_]) && ($ECI[$_] eq $ECIII[$_])&& ($ECI[$_] eq $HEL[$_])) {
					print INVAR "$position\t$ECI[$_]\t$ECII[$_]\t$ECIII[$_]\t$HEL[$_]\n";
					$position++; 
					}
					else {
					print SNPs "$position\t$ECI[$_]\t$ECII[$_]\t$ECIII[$_]\t$HEL[$_]\n";
					$position++;
					$SNPs++;
					}
				}
