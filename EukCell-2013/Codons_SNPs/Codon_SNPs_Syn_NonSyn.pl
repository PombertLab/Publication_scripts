...
## Query each SNP-containing codon against the hash to verify if the resulting amino acids are the same.
## Write synonymous changes to .syn, non-synonymous changes to non-syn.
## The codon hash table can be modified to reflect alterations to the genetic code. 
	...
	open NONSYN, ">$snps.nonsyn";
		...
		while (my $line = <IN>) {
			...
			my $strain3 = $5;
				...
				print SYN "$gene\t$position\t$aa{$strain1}\t$aa{$strain2}\t$aa{$strain3}\n";
				
				}
				
				else {
				print NONSYN 	"$gene\t$position\t$aa{$strain1}\t$aa{$strain2}\t$aa{$strain3}\n";
				}
