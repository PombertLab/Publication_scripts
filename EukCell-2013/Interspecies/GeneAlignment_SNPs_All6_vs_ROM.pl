...
my $usage = 'perl GeneAlignment_SNPs_All6_vs_ROM.pl list.txt';
	...
	open IN, "<$txt" or die "cannot open $txt";
		...
		while (my $line = <IN>) {
			...
			if ($line =~ /^(ECU\d+.*)/) { ## match the query to fit the desired gene names in the list provided
				...
				open INVAR, ">$ECU.rom.invar";
				open SNPs, ">$ECU.rom.snps";
				open GAPs, ">$ECU.rom.gaps";
				open MET, ">$ECU.rom.metrics";
				...
				for (0..($end-1)) {
					...
					if (($ECI[$_] eq '-') || ($ECII[$_] eq '-') || ($ECIII[$_] eq '-')|| ($ROM[$_] eq '-')) {
					print GAPs "$position\t$ECI[$_]\t$ECII[$_]\t$ECIII[$_]\t$ROM[$_]\n";
					$position++; 
					}
					elsif (($ECI[$_] eq $ECII[$_]) && ($ECI[$_] eq $ECIII[$_])&& ($ECI[$_] eq $ROM[$_])) {
					print INVAR "$position\t$ECI[$_]\t$ECII[$_]\t$ECIII[$_]\t$ROM[$_]\n";
					$position++; 
					}
					else {
					print SNPs "$position\t$ECI[$_]\t$ECII[$_]\t$ECIII[$_]\t$ROM[$_]\n";
					$position++;
					$SNPs++;
					}
				}
